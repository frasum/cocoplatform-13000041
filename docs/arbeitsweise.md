...
## §112 — SE1: Sentry-Selbsttest formalisiert (29.07.)

Abnahme-Anker: HEAD 903e297a9 — vier Gates grün (tsc 0 · eslint --max-warnings=0 0 · prettier clean · vitest 1990/1991, 197 Dateien; +1 Skip Bestand).

**Befund vor dem Bau (Prozess-Lehre, wichtiger als das Feature):** Am 29.07. lagen zwei Sentry-Testfehler aus `cocoplatform.online` vor (Server COCO-E, Client COCO-D, 07:25 UTC) — der auslösende Code existierte auf `main` NICHT und auch in keinem Commit der Historie. Der Bauherr hatte die Knöpfe in Lovable gebaut und publiziert, ohne dass der Commit auf main landete. Damit lief Produktion mit Code, der im Repo nicht existiert. Neue Regel: **Publish ohne Commit ist ein Zustand, den wir aktiv suchen, nicht einer, auf den wir warten.** Die Wiedereinstiegs-Regel (`git log` seit Anker) fängt undokumentierte Commits — sie fängt NICHT undokumentierte Deployments. Gegenmittel: bei jedem Wiedereinstieg mit Produktions-Belegen (Sentry-Screenshot, Feldmeldung) prüfen, ob der belegte Code überhaupt auf main liegt; nach jedem Lovable-Publish gegen `origin/main` gegenprüfen. Bestandsmeldung zu diesem Auftrag: der im Prompt genannte Anker 47a6d5a4 war überholt — die Test-Button-Commits (4445db86f, 2f3ea7548) waren zwischenzeitlich auf main gelandet; Arbeitsbaum sauber, keine unversionierten Änderungen.
