...
## §125 — Statistik-Vormittag: zehn Runden PDF-Ausbau + BWA-USt-Korrektur (02.08.)

**Abnahme-Anker:** HEAD `ac451209` — vier Gates grün (`tsc` 0 · `eslint` 0 · `prettier` clean · `vitest` 222 Dateien / 2253 Tests, 0 Skips; 2199 + 54). 107 Commits seit `ab8dfc4e`, alle zehn Runden vom Prüfer inhaltlich abgenommen.

**STAT2d — Trinkgeld-Quote:** `tipRatePct` auf **HAUS**-Umsatz (Bauherren-Entscheid Variante b: Takeaway-Anteile verzerren sonst den Standortvergleich). Kachel im Standortvergleich + PDF-Spalte; Deltas als **PROZENTPUNKTE** (`ppDelta`) statt relativem Wachstum — inkl. Nachtrag: auch die Vergleichszeile spricht pp („YUM +0,1 pp vs. spicery“), eine Kachel, zwei Delta-Sprachen beseitigt. Erste Live-Erkenntnis: YUM-Gäste geben relativ mehr (9,0 vs. 8,9 %), spicery −0,3 pp im Trend — von den Absolutkacheln unsichtbar.

**STAT3d–3g — PDF-Reifung am gerenderten Ergebnis** (Bauherren-QA-Schleife, jeweils Befund am Render → Runde): **3d** Beträge kaufmännisch auf ganze Euro (NUR Formatschicht; Quoten/Deltas behalten die Nachkommastelle; ±1-€-Fußnote — Summen werden NICHT passend gemacht). **3e** Linien-Achse geschnitten (Baseline sichtbar beschriftet; Balken bleiben 0-basiert), `niceTicks` 1/2/5-Raster, Kurzlabel „Direkt (Tel./Abholung)“, „Pers.-Quote“, Delta-Färbung (gedeckte Töne, nur Veränderungswerte). **3f** 5-Jahres-Grafik **KUMULIERT** Jan–&lt;M&gt; je Standort (`ytd-compare`-Modul, MB3-Klemmregel: laufender Monat ⇒ alle Jahre bis M−1; Lückenjahr ⇒ `null` statt Teilsumme) — ersetzt die Trinkgeld-Matrix (Klarnamen-Datenschutz + Kernwerte stehen in der Tabelle). **3g** Reihenfolge Zahlen → Grafiken nach Zeithorizont (Tag/Jahr/5 Jahre), Trinkgeld-**EINZEILER** mit Bereichs-Quoten (Selbsttest: Zeilen-Gesamtquote ≡ Tabellen-TG-Quote, eine Formel), `axisFor`-Regel „oberster Tick ≥ Maximum“ auf ALLE vier Geometrien ausgeweitet (Prüfer-Erweiterung: der Filter-Fehler steckte in allen Charts, nicht nur grouped), `BLOCK_GAP` vereinheitlicht, Farb-Registrierung vor dem ersten Block (stabile Standortfarben).

**STAT3h — Modell-Ergebnis vor Steuern:** `estimatedPreTaxResultCents` = `db × (Kassenumsatz/Faktor − BE-Monat netto)` — NICHT die nackte Differenz (oberhalb des BE zählt nur der Deckungsbeitrag). Zeile nur bei abgeschlossenem Kalendermonat + Alle-Standorte + berechenbarem BE; Fußnote kennzeichnet die Modellrechnung (ersetzt nicht die Steuerbüro-BWA).

**STAT3i — Render-Bug + Layout-Finale:** Ursache Sperrsatz war **U+2212** im Zeilentext (jsPDF/Helvetica kennt nur WinAnsi; Lovable-Diagnose widerlegte die Prüfer-Vermutung Font-State — §104-Vorbild). Wiederverwendbarer Test-Helfer `src/test/win-ansi.ts` (`assertWinAnsiSafe`: U+2212/Δ/EN-/EM-Dash/alles > U+00FF, sprechende Fehlermeldungen) auf Trinkgeld- UND Ergebnis-Zeile; Breiten-Test mit Maximalwerten; beide Einzeiler ans Ende (nach der 5-Jahres-Grafik, vor die Fußnoten); **EINE** Legende (Tagesumsatz) statt drei.

**BWA-V1 — USt-Zuordnung Haus-Speisen** (Bauherren-Fund, materiell): Seit 01.01.2026 gilt 7 % auch für Speisen IM Haus; `computeBreakEven` rechnete sie mit 19 % — Mix 16,3 % statt real ~9,8 % (Kontrollrechnung Mai-BWA), das Modell-Ergebnis war dadurch **ETWA HALBIERT** ausgewiesen. Fix: Konstante `INHOUSE_FOOD_REDUCED_FROM="2026-01"`, USt **monatsgenau** je Zeile (Mischperiode 2025/2026 korrekt gewichtet), neues Feld `factorCurrent` (Regelstand der Gegenwart) für die STAT3h-Netto-Umrechnung — Misch-`factor` bleibt für Perioden-Brutto. Charakterisierungstests: reine Alt-Periode bit-identisch (für 2025 war der Code korrekt), Neu-Periode, Mischperiode mit Handrechnung, `factorCurrent`-null-Kette. KEIN versendeter Bericht enthielt die falsche Zahl (Feature war jünger als der Fund). BWA-Beleg: Steuerbüro führt „Speisen im Haus“/„außer Haus“ durchgehend getrennt; Bauherren-Sichtprüfung am Bildschirm: Mix ~9,8 % erwartet.
...
**UI2 (`06131abf`):** Kassen-Kopfzeile mit 4-px-Akzentleisten (Sektions-Muster der Seite), Events blau / Wetter amber getönt, Titelzeilen mit Icon. **IMPACT-ESKALATION:** `noticesTone` (pure, getestet) färbt die Events-Karte nach höchstem anstehendem Impact — `info`(blau) / `hoch` ⇒ `warning`(amber) / `sehr_hoch` ⇒ `danger`(rot + Warndreieck); FERIEN-Notices eskalieren **NIE** (kein Impact — Dauerkontext färbt nicht). Wiesn-Vortag 18.09. wird der erste echte Rot-Fall.

**ZT1 (`f092b02e`) — Lohn-Sichtbarkeit auf admin/payroll verengt (Bauherren-Auftrag):** Die fünf Tabs der Arbeitszeitübersicht (Zusammenfassung/Buchhaltung/Perioden/Brutto-Netto/Provision) waren für MANAGER sichtbar (nur `planer` ausgenommen) — jetzt `canSeePayrollTabs = admin||payroll`, `effectiveTab` erzwingt Wochenplan ohne Recht. Stammdaten-Tab (`staff.$staffId`) defensiv doppelt gegated (SD1-Guard schützte die Route bereits). **SERVERSEITIG** neun Read-/Write-Functions von `manager` auf `["admin","payroll"]` verengt (SFN-Overview, PayrollNotes, Advances, Absences, RecurringNotes inkl. Schreib-Fns); `getStaff` ebenfalls verengt — benannte Teilrevision der SD1-Erweiterung vom 18.07. (einziger Aufrufer ist die admin/payroll-Seite; Bauherren-Auftrag deckt die Revision). **BEWUSST offen geblieben** (gemeldeter Konflikt): `getTimeOverview(+Batch)` bleibt `manager` — speist den Wochenplan (Schichtzahlen/Perioden-Summen); `listPeriods`, `listStaff`, Wochenplan-Schreib-Fns unverändert. Lohn-Queries der Seite zusätzlich `enabled: canSeePayrollTabs` (kein 403 im Wochenplan). ⚠ **Offene Sichtprüfung:** Manager-Kennung — Wochenplan lädt, Zeiten anlegen/ändern/löschen, Konsole ohne Forbidden (Lovable konnte nur admin testen, ehrlich gemeldet).
